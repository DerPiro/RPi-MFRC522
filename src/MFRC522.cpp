...
void delay(long long ms) {
  std::this_thread::sleep_for(std::chrono::duration<long long, std::milli>(ms));
}
 ...
 */
MFRC522::MFRC522(int ss_pin, int rst_pin, int spi_channel)
    : ss_pin{ ss_pin }
    , rst_pin{ rst_pin }
    , channel{ spi_channel }
    , uid{} {
...
void MFRC522::PCD_ReadRegister(
  ...
  // bytes from register."));
  byte address
      = 0x80 | (reg & 0x7E); // MSB == 1 is for reading. LSB is not used in
                             // address. Datasheet section 8.1.2.3.
  byte index = 0;            // Index in values array.
  count--;                   // One read is performed outside of the loop
  byte buffer = address;
  wiringPiSPIDataRW(channel, (unsigned char*) &buffer, 1);
  while (index < count) {
    if (index == 0
  ...
  buffer = 0;
  wiringPiSPIDataRW(channel, (unsigned char*) &buffer, 1);
  values[index] = buffer; // Read the final byte. Send 0 to stop reading.
} // End PCD_ReadRegister()
...
byte MFRC522::PCD_CalculateCRC(
  ...
  while (1) {
    ...
    if (--i
        == 0) { // The emergency break. We will eventually terminate on this one
...
void MFRC522::PCD_Reset() {
  ...
  // oscillator start-up time is the start up time of the crystal + 37,74�s. Let
  // us be generous: 50ms.
  delay(50);
...
byte MFRC522::PCD_CommunicateWithPICC(
  ...
  while (1) {
    ...
    if (--i == 0) { // The emergency break. If all other condions fail we will
 ...
 * or three iterations are used: UID size	Number of UID bytes		Cascade levels
 * Example of PICC
 * 		========	===================		==============		===============
...
byte MFRC522::PICC_Select(
  ...
  //		Byte 1: NVB					Number of Valid Bits (in complete command, not just
  // the UID): High nibble: complete bytes, Low nibble: Extra bits. 		Byte 2:
  // UID-data or CT		See explanation below. CT means Cascade Tag. 		Byte 3:
  // UID-data 		Byte 4: UID-data 		Byte 5: UID-data 		Byte 6: BCC Block
  // Check Character - XOR of bytes 2-5 		Byte 7: CRC_A 		Byte 8: CRC_A
  ...
  while (!uidComplete) {
    ...
    byte bytesToCopy = currentLevelKnownBits / 8
                       + (currentLevelKnownBits % 8
                              ? 1
                              : 0); // The number of bytes needed to represent
    ...
    while (!selectDone) {
      ...
      // Set bit adjustments
      rxAlign = txLastBits; // Having a seperate variable is overkill. But it
...
byte MFRC522::PICC_HaltA() {
  ...
  // the end of the frame containing the 		HLTA command, this response shall be
 ...
 */
const std::string
MFRC522::GetStatusCodeName(byte code ///< One of the StatusCode enums.
) {
  switch (code) {
    case STATUS_OK:
      return ("Success.");
 ...
 */
const std::string
MFRC522::PICC_GetTypeName(byte piccType ///< One of the PICC_Type enums.
) {
  switch (piccType) {
...
void MFRC522::PICC_DumpMifareClassicSectorToSerial(
  ...
  //		g[3]	Access bits for the sector trailer, block 3 (for sectors 0-31) or
  // block 15 (for sectors 32-39) 		g[2]	Access bits for block 2 (for sectors
  // 0-31) or blocks 10-14 (for sectors 32-39) 		g[1]	Access bits for block 1
  // (for sectors 0-31) or blocks 5-9 (for sectors 32-39) 		g[0]	Access bits
  // for block 0 (for sectors 0-31) or blocks 0-4 (for sectors 32-39)
  ...
  for (char blockOffset = no_of_blocks - 1; blockOffset >= 0; blockOffset--) {
    ...
    if (isSectorTrailer) {
      status = PCD_Authenticate(PICC_Command::PICC_CMD_MF_AUTH_KEY_A,
                                firstBlock, key, uid);
      if (status != STATUS_OK) {
...
bool MFRC522::MIFARE_OpenUidBackdoor(bool logErrors) {
  ...
  byte validBits = 7; /* Our command is only 7 bits. After receiving card
       response, this will contain amount of valid response bits. */
  ...
  if (status != STATUS_OK) {
    if (logErrors) {
      printf(
          "Card did not respond to 0x40 after HALT command. Are you sure it is "
          "a UID changeable one?");
      printf("Error name: ");
  ...
  if (status != STATUS_OK) {
    if (logErrors) {
      printf(
          "Error in communication at command 0x43, after successfully "
          "executing 0x40");
      printf("Error name: ");
      printf("%s\n", GetStatusCodeName(status).c_str());
    }
...
bool MFRC522::MIFARE_SetUid(byte* newUid, byte uidSize, bool logErrors) {
  ...
  byte status = PCD_Authenticate(PICC_Command::PICC_CMD_MF_AUTH_KEY_A, (byte) 1,
                                 &key, &uid);
  if (status != STATUS_OK) {
    if (status == STATUS_TIMEOUT) {
      ...
      if (!PICC_IsNewCardPresent() || !PICC_ReadCardSerial()) {
        printf(
            "No card was previously selected, and none are available. Failed "
            "to set UID.");
        return false;
      }

      status = PCD_Authenticate(PICC_Command::PICC_CMD_MF_AUTH_KEY_A, (byte) 1,
                                &key, &uid);
      if (status != STATUS_OK) {
        // We tried, time to give up
        if (logErrors) {
          printf(
              "Failed to authenticate to card for reading, could not set "
              "UID: ");
          printf("%s\n", GetStatusCodeName(status).c_str());
        }
        return false;
