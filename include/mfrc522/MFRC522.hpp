...
enum PICC_Command {
  ...
  // http://www.nxp.com/documents/data_sheet/MF1S503x.pdf, Section 9) Use
  // PCD_MFAuthent to authenticate access to a sector, then use these commands
  // to read/write/modify the blocks on the sector. The read/write commands
  ...
  PICC_CMD_MF_DECREMENT = 0xC0, // Decrements the contents of a block and stores
                                // the result in the internal data register.
  PICC_CMD_MF_INCREMENT = 0xC1, // Increments the contents of a block and stores
                                // the result in the internal data register.
...
public:
  ...
  /////////////////////////////////////////////////////////////////////////////////////
  MFRC522(int ss_pin = 24, int rst_pin = 25, int spi_channel = 0);
  /////////////////////////////////////////////////////////////////////////////////////
  ...
  const std::string GetStatusCodeName(byte code);
  byte              PICC_GetType(byte sak);
  // old function used too much memory, now name moved to flash; if you need
  // char, copy from flash to memory
  // const char *PICC_GetTypeName(byte type);
  const std::string PICC_GetTypeName(byte type);
